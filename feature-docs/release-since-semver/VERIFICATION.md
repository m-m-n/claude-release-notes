...
- [ ] MT-1 (TS-9): Run a case-insensitive search over the three modified files
...
- [ ] MT-2 (TS-9): Read each of the fifteen locations against task0001's
...
- [ ] MT-5: Confirm the change set contains only the three documentation files
...
- [ ] MT-6: Report — do not fix — any stale statement found outside the
      fifteen enumerated locations, in particular
      `feature-docs/release-notes-fetcher/IMPLEMENTATION.md`'s Shared
      Components row for the differential fetch (IMPLEMENTATION.md Open
      Questions).
