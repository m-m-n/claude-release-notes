...
| `github.Client` | Fetch releases | Constructor takes token and API base URL (overridable for tests). `Latest()` post: the newest non-draft, non-prerelease release. `Since(version)` post: the non-draft, non-prerelease releases from the first listing page only that compare as a newer semantic version than `version`, newest first; empty slice when up to date. Release value has fields TagName, Body, PublishedAt. | task0003 (provides), task0006 (consumes) |
