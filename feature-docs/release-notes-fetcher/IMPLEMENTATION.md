...
| `state.Store` | Persist the semver-greatest delivered version | Constructor takes the state directory (caller resolves XDG; empty string means "resolve XDG default internally"). `Load` post: returns (version, found=true) or (found=false) when no state exists — never an error for mere absence. `Save(version)` post: state persisted atomically (temp file + rename), directories created as needed. | task0002 (provides), task0006 (consumes) |
| `github.Client` | Fetch releases | Constructor takes token and API base URL (overridable for tests). `Latest()` post: the first entry in the listing, in listing order, that is not a draft, not a prerelease, and whose tag name parses as `vMAJOR.MINOR.PATCH`; exhausting pagination without such an entry yields the existing "no releases found" error. `Since(version)` post: the non-draft, non-prerelease releases from the first listing page only that compare as a newer semantic version than `version`, newest first; empty slice when up to date. Release value has fields TagName, Body, PublishedAt. | task0003 (provides), task0006 (consumes) |
